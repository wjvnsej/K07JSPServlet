...
<body>
	...
	<h2>리퀘스트 영역에 저장 된 속성 삭제 후 출력하기</h2>
	<%
	...
	<%-- 
	최초 요청된 페이지와 포워드로 전달 된 페이지는 request 영역을 공유하게 된다.
	URL주소창에는 최초 요청된 페이지의 경로명이 보여지지만, 출력되는 페이지는
	새롭게 전달된 페이지의 내용이 출력되게 된다.
	그러므로 사용자는 페이지 이동을 인지하지 못하지만, 내부적으로 페이지전달(포워드)
	된것이다. 쿼리스트링 형태로 파라미터도 전달 할 수 있다.
	--%>
	...
	웹 브라우저의 URL주소창에는 새롭게 요청된 페이지의 경로명이 보여지게 된다.
	--%>
