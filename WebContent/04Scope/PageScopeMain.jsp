...
<%
/* 
모든영역(페이지, 리퀘스트, 세션, 어플리케이션)에는 모든 타입의 객체를
저장 할 수 이싿. Object타입으로 자동 형 변환 되어 저장되고, 사용시에는
각 객체로 형 변환 후 사용하면 된다. 차후EL(표현언어)을 사용하면 형 변환 없이
사용할 수 있다.

1. page영역
	- 페이지영역에 저장된 속성은 해당 페이지에서만 공유되고,
	페이지를 벗어나는 순간 소멸된다.
	- 페이지 영역에는 pageContext 내장객체를 사용하여 
	영역을 사용한다.
*/
pageContext.setAttribute("pageNumber", 1000);
pageContext.setAttribute("pageString", "페이지영역에 저장할 문자열");
...
<body>
   ...
   <%
   /* 
   page영역에 저장될 때 Object 타입으로 자동 형 변환 되어 저장되므로
   읽어올 때도 Object 타입으로 가져오는 것이 기본이다.
   */
   Object obj = pageContext.getAttribute("pageDate");
   String dateString ="";
   //Date타입으로 형 변환 가능한지 확인 후..
   if(obj instanceof Date){
	   //가능하다면 Date 타입으로 형 변환 한다.
   ...
   <%-- 
   		페이지영역에 저장 된 속성값은 새로운 페이지에 대한 요청이 발생하면
   		속성값이 모두 파과되어 소멸된다. 즉  다른페이지와는 속성이 공유되지 않는다.
   --%>
   <h2>페이지 이동</h2>
   ...
   <br />
   <%-- 
   		외부파일을 인클루드 하는 경우 외부문서는 원본그대로 현재문서에
   		포함된 후 컴파일 되므로 include 지시어를 사용하는 경우
