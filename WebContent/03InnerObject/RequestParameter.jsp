...
폼값을 받을 수 있다. 만약 값이 입력되지 않았다면 길이가 0인 String 객체를
반환하고, 파라미터명이 틀린 경우에는 null을 반환한다.
*/
...
if( !(name == null || name.equals("")) ) {
	...
	아래와 같이 사용할 수 있다. 또는 문자열을 연결하여 사용할 수도 있다.
	*/
	out.println("<script>");
	out.println("	alert('이름을 입력하세요');");
	/* out.println("	history.back();");
	out.println("</script>"); */
	String jsStr = "history.back();" + "</script>";
	out.println(jsStr);
}

/* 
radio는 폼 값을 아래 2가지 방법을 모두 사용할 수 있다.
항목은 여러개이나, 실제 선택가능한 항목은 하나이기 때문이다.
*/
//방법1 : text타입과 동일한 형태로 폼 값을 받을 때
...
여러개의 항목 중 체크가 된 항목만 전송된다. 파라미터명이 틀리거나
...
웹 브라우저에 출력할 때는 특수기호를 <br>태그로 변환 후 출력해야 한다.
*/
String self_intro = 
